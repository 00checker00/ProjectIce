...
public class ShapeEditInputProcessor extends InputAdapter {
    ...
    private final Shape emptyShape = new Shape() {{
        closed = true;
    }};
    ...
    public boolean touchDown(int x, int y, int pointer, int button) {
        ...
        if (button == Input.Buttons.MIDDLE && canvas.selectedModel != null) {
            canvas.selectedModel.startPos = canvas.screenToWorld(x, y);
        }
        ...
        Array<Vector2> vs = lastShape.vertices;
    ...
    public boolean mouseMoved(int x, int y) {
        ...
        canvas.selectedModel.mousePos = canvas.screenToWorld(x, y);
        canvas.selectedModel.findPath();
    ...
    private List<Vector2> getAllPoints() {
        ...
        for (Shape shape : model.shapes)
            for (Vector2 vertex : shape.vertices)
                points.add(vertex);
        return Collections.unmodifiableList(points);
    }
}
