	...
	private BufferedImage extractImage (BufferedImage page, Region region, File outputDirFile, int padding) {
		...
		if (region.rotate) {
			...
			if (region.width == 0 || region.height == 0)
				splitImage = new BufferedImage(1, 1, page.getType());
			else
				splitImage = page.getSubimage(region.left, region.top, region.width, region.height);
		}

		// draw the image to a bigger one if the original size differs from the packed one
		if (region.originalWidth != region.width || region.originalHeight != region.height) {
			BufferedImage origSizeImage = new BufferedImage(region.originalWidth, region.originalHeight, page.getType());
			Graphics2D g2 = origSizeImage.createGraphics();
			//g2.drawImage(splitImage, (int) region.offsetX, (int) region.offsetY, null);
			g2.drawImage(splitImage, (region.originalWidth-region.width)/2, (region.originalHeight-region.height)/2, null);
			g2.dispose();
			splitImage = origSizeImage;
		}
